...
```{r}
wc_regions <- st_read(here("data/wc_regions_clean.shp"))
```
-   read in SST rasters
...
```{r}
# Ruth read the data in using list.files... let's try it that way
#--- you start with a bunch of tif files that you want to stack
#--- I created a list of just the ones starting with the letter a to get the sst rasters
...
#--- at this point you get a crs that says like epsg4326 AND epsg9122 so we'll have to change that
```
...
#### SB depth data
Can I do a quick surfing analysis with the depth data?
maybe the data is not of high enough resolution, but we should see if we can crop it close to the santa barbara area ... the depth is not at a high enough resolution to infer anything surf related
...
maybe we can also visualize county economic data in the same map! Some species are really cheap to purchase, while others are really expensive.  If we really want to understand where the best location would be for aquaculture, we should also understand where the seafood would sell the best based on economic information of that region.  If we want to create a sustainable seafood industry, sourcing local protein will be crucial in reducing transportation emissions. The American Lobster has the potential to be a very expensive commodity, because wild lobster in California runs at a ludicrous price right now.  I would like to add economic information by county (such as median household income) so that we can pick an area of California with lots of rich people who would want to buy lobster. If we can visualize this in the same map it would be useful in picking the right location for aquaculture.  

```{r}
#devtools::install_github("UrbanInstitute/urbnmapr")
library(urbnmapr)
###---- hmmm this package does not contain geometries so idk how to plot it
...
library(tidycensus)
#----had to obtain API key from
#http://api.census.gov/data/key_signup.html #---actually didn't have to use it haha
...
#--- we eventually want to be able to stack the raster with our spatraster from earlier called oyster_habitat
#--- we have to change the crs, change class??, and then crop it to the same dimensions

tm_shape(ca_income) + tm_fill("estimate")
```
